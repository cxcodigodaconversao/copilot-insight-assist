...
const ConviteInput = z.object({
  ...
  nome: z.string().min(1),
  role: z.enum(["closer", "sdr"]),
  redirectTo: z.string().url().optional(),
});

/** Convida um closer ou SDR por e-mail. Somente o líder pode chamar. */
export const convidarMembro = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => ConviteInput.parse(d))
  .handler(async ({ data, context }) => {
    const { data: ehLider, error: erroPapel } = await context.supabase.rpc("has_role", {
      _user_id: context.userId,
      _role: "lider",
    });
    if (erroPapel) throw new Error("Não foi possível verificar suas permissões.");
    if (!ehLider) throw new Error("Apenas o líder pode convidar pessoas.");
